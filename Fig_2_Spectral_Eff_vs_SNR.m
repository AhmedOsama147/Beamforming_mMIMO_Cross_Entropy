...
for k = 1:lenSNR
    ...
    [FRF_OMP,FBB_OMP] = OMP(Fopt,NT_RF,NT/NT_RF,AT,NS);
    qFRF_OMP = QuantizePhasebit2(FRF_OMP,NT);
    Capacity_OMP2(i) = log2(det(eye(NR)+(EN(k)/NS)*H_n*qFRF_OMP*FBB_OMP*FBB_OMP'*qFRF_OMP'*H_n'));
    
    %     
    [FRF_PE,FBB_PE] = AltMinPE(Fopt,NT_RF,NT,NS);
    qFRF_PE = QuantizePhasebit2(FRF_PE,1);
    Capacity_PE2(i) = log2(det(eye(NR)+(EN(k)/NS)*H_n*qFRF_PE*FBB_PE*FBB_PE'*qFRF_PE'*H_n'));   
    
    K = 150; T = 40;
    [FBB_CE_opt,FRF_CE_opt] = cross_entropy(Fopt,K,T,B(2),NS,NT_RF,NT);
    Capacity_CE2(i) = log2(det(eye(NR)+(EN(k)/NS)*H_n*FRF_CE_opt*FBB_CE_opt*FBB_CE_opt'*FRF_CE_opt'*H_n'));   
...
plot(SNR,cap_full_digital(:),"Color",'k','LineWidth',2,'Marker','diamond');
hold on;
plot(SNR,cap_CE2(:),"Color",'r','LineStyle','--','LineWidth',2,'Marker','square');
hold on;
plot(SNR,cap_OMP2(:),"Color",[0.9 0.5 0.5],'LineStyle',':','LineWidth',2,'Marker','o');
hold on;
plot(SNR,cap_PE2(:),"Color",'b','LineStyle','--','LineWidth',2,'Marker','.');
hold on;
legend('Fully Digital','Machine Learning (FC)', 'PE-AltMin','OMP Fully Connected',...
